...
**URL:** http://elbo-spice.gs.washington.edu/shiny/burrito
...
1) Download the example data by clicking and unzipping [**this file**](https://cnoecker.github.io/data/BurritoData.zip). See below for more details on these datasets.

2) Navigate your browser to http://elbo-spice.gs.washington.edu/shiny/burrito
...
3) **Function attribution table (not shown here):** BURRITO can also accept what it calls a "functional attribution table", which is a contribution table in the format produced by PICRUSt v1, describing the abundance of each gene (KEGG ortholog), from each taxon, in each sample. A stratified functional abundance table as produced by the Humann2 software could also be used for this purpose, with some re-formatting. 
...
To do so, run the following commands in your qiime2 environment (which were also provided in the pre-workshop instructions). 
...
### Using BURRITO
Navigate your web browser to the BURRITO website: http://elbo-spice.gs.washington.edu/shiny/burrito You should see a panel of data input options. 
